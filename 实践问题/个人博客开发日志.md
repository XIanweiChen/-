...
9.4 从github上读取目录做好了,接下来要做一个加载的动画以及默认选中
...
Promise.all()完成的时间顺序不一样,最后进行一次排序才能喝github上的顺序一样
...
```
ln -s /www/node-v10.16.3-linux-x64/bin/node /usr/bin/node
ln -s /www/node-v10.16.3-linux-x64/bin/npm /usr/bin/npm
```
